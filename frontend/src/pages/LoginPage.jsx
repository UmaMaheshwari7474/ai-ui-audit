...
export const LoginPage = () => {
  ...
  const handleGoogleLogin = async () => {
    ...
    // Open a realistic mock popup delay
    setTimeout(async () => {
      try {
    ...
    <div className="min-h-screen bg-background-light dark:bg-background-dark flex flex-col items-center justify-center p-4 transition-colors">
      ...
      <Card className="w-full max-w-md p-8 relative z-10">
        <div className="text-center mb-6">
          <h2 className="text-lg font-bold text-slate-800 dark:text-white">Sign In to Dashboard</h2>
          <p className="text-xs text-slate-500 mt-1">Access your audit history and submit new reviews.</p>
        </div>
      ...
      {/* Demo helper notice */}
      <div className="mt-6 text-center max-w-xs text-[10px] text-slate-400 dark:text-slate-500 font-medium">
        <p>💡 Tip: You can register a new account, or log in using any random email and password to begin right away.</p>
      </div>
    </div>
  );
};
